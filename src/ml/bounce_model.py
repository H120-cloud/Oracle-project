"""
ML Bounce Prediction Model — V2

Gradient Boosting classifier that predicts bounce probability.
...
MODEL_NAME = "bounce_predictor"


class BounceModel:
    """ML-enhanced bounce probability predictor with cold-start fallback."""
    ...
    def _load_model(self):
        ...
        if payload is not None:
            ...
            logger.info("Bounce model loaded (trained)")
        else:
            logger.info("No trained bounce model found — cold-start mode")

    @property
    def is_trained(self) -> bool:
        return self._is_trained

    def predict(self, features: BounceFeatures, rule_based_prob: float) -> float:
        """
        Return bounce probability (0–100).
        ...
        try:
            X = FeatureEngineer.bounce_to_array(features).reshape(1, -1)
            ...
            logger.debug(
                "BounceModel predict: ml=%.1f rule=%.1f blended=%.1f",
                ml_prob, rule_based_prob, blended,
            )
            return round(blended, 1)

        except Exception as exc:
            logger.error("BounceModel predict failed, fallback to rule-based: %s", exc)
            return rule_based_prob
    ...
    def train(
        ...
        X_train: shape (n_train_samples, n_bounce_features)
        y_train: binary array (1 = successful bounce, 0 = failed bounce)
        X_test: shape (n_test_samples, n_bounce_features) - held-out test set
        ...
        logger.info(
            "Bounce model trained: samples=%d, train_acc=%.3f, test_acc=%.3f",
